...
featuresOfThePlot <- function(df) {
  filtered <- df %>%
    filter(str_length(LoopPattern) <= 8) %>%
...
pairsMFEPlot <- function(df) {
  ...
  mfe <- df %>%
    ...
    ggplot(aes(x = RnaFoldMFE, group = Tipo)) +
    ylab("Densidad") +
...
purinePlot <- function(df) {
  ...
  myPlot <- df %>%
    ...
    ggplot(aes(x = PurinePercentPairs, group = Tipo)) +
    ylab("Densidad") +
